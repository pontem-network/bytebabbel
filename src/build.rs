...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let script_path = root.join(PathBuf::from(format!("./{TEST_CONTRACTS_DIR}")).join(BUILD_CONTRACTS_SH));
	let output_path = source_path.join(BUILD_CONTRACTS_DIR);


	let status = Command::new(script_path).current_dir(&root).envs(env::vars()).status()?;
	if !status.success() {
	...
	// list all files to watch:
	{
		...
		let glob = Glob::new("**/*.{sol,bin,evm,sol_json.ast}").map_err(map_err_warn)
		                                                       .unwrap_or_else(|_| unreachable!("sol files not found"));
		let walker = glob.walk_with_behavior(&source_path, LinkBehavior::ReadTarget)
		                 .not(["**/.*/**"])
		...
		for entry in walker {
			...
			let path = entry.path();
			if path.display().to_string().contains(&output_path.display().to_string()) {
				let path = PathBuf::from(&path.display().to_string()[(root.display().to_string().len() + MAIN_SEPARATOR_STR.len())..]);
				let stem = path.file_stem().expect("file name").to_string_lossy().replace(".", "_").replace(" ", "_");
